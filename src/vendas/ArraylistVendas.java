...
public class ArraylistVendas {
		...
		try (Scanner sc = new Scanner(System.in)) {
			...
			while (true) {
				...
				if (option == 1) {
					...
					consultarItem(produtos, precos, estoque, histven); //chama o método consultar item
				}
					...
					mostrarRelatorio(produtos, precos, histven, fat, faturamento); //chama o método mostrar relatório
				}
	...
	public static void venderItem(ArrayList<String> produtos, ArrayList<Double> precos, ArrayList<Integer> estoque, ArrayList<Integer> histven, ArrayList<Double> fat, Scanner sc) {
		System.out.print("\nInsira o código do produto que deseja vender\n "+ produtos+": ");

		int indice = sc.nextInt(); //lê o código do item
		
		// String venda = sc.nextLine();
		// int indice = produtos.indexOf(venda); //esquema para ler o produto pelo nome caso quisesse

		if (indice <= -1 || indice >= produtos.size()) {
			System.out.println("Produto não encontrado!"); //se o número digitado for menor que -1 OU maior igual o máximo do loop (size é loop + 1), cancela a operação
			...
			if (venda1 > estoque.get(indice)) {
				System.out.println("\nnão é possível vender mais unidades que o estoque disponível"); //se a quantidade de venda for maior que o estoque cancela
			} else if (venda1 <= 0) {
				System.out.println("\nnão é possível vender itens iguais ou inferiores a 0"); //se a quantidade for menor igual a 0 cancela
			} else {
	...
	public static void mostrarRelatorio(ArrayList<String> produtos, ArrayList<Double> precos, ArrayList<Integer> histven, ArrayList<Double> fat, double faturamento) {
